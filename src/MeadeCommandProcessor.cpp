...
//      Get Site Longitude
//      Returns: DDD*MM#
//               Where DDD is the longitude in degrees and MM the minutes. Longitudes are from 0 to 360 going WEST. so
//               179W is 359 and 179E is 1.
//
// :Gc#
...
//      Get UTC offset time
//      Returns: sHH#
//               Where s is the sign and HH are the number of hours that need to be added to local time to convert to
//               UTC time
//
// :Ga#
...
//                 |    +-------------------------------------------- [1] The motion state.
//                 |                                                      First character is RA slewing state ('R' is
//                 East, 'r' is West, '-' is stopped). |                                                      Second
//                 character is DEC slewing state ('d' is North, 'D' is South, '-' is stopped). | Third character is TRK
//                 slewing state ('T' is Tracking, '-' is stopped). | * Fourth character is AZ slewing state ('Z' and
//                 'z' is adjusting, '-' is stopped). |                                                      * Fifth
//                 character is ALT slewing state ('A' and 'a' is adjusting, '-' is stopped).
//                 +------------------------------------------------- [0] The mount status. One of 'Idle', 'Parked',
//                 'Parking', 'Guiding', 'SlewToTarget', 'FreeSlew', 'ManualSlew', 'Tracking', 'Homing'
...
//      Where DDD the nmber of degrees (0 to 360), MM is minutes. Longitudes are from 0 to 360 going WEST. so 179W is
//      359 and 179E is 1. Returns: 1 if successfully set, otherwise 0
//
// :SGsHH#
//      Set Site UTC Offset
...
//      Where n.nn is a signed floating point number representing the number of arcminutes to move the mount left or
...
// :Qd#
//      Stop Slewing
//      Stops slew in specified direction where d is n, s, e, w, a (the first four are the cardinal directions, a stands
//      for All). Returns: nothing
//
...
//            <Stepper Info> is a pipe-delimited string of Motor type (NEMA or 28BYJ), Pulley Teeth, Steps per
//            revolution) <GPS info> is either NO_GPS or GPS, depending on whether a GPS module is present <AzAlt info>
//            is either NO_AZ_ALT, AUTO_AZ_ALT, AUTO_AZ, or AUTO_ALT, depending on which AutoPA stepper motors are
//            present <Gyro info> is either NO_GYRO or GYRO depending on whether the Digial level is present
...
MeadeCommandProcessor::MeadeCommandProcessor(Mount *mount, LcdMenu *lcdMenu)
{
    ...
    // In case of DISPLAY_TYPE_NONE mode, the lcdMenu is just an empty shell class to save having to null check
    // everywhere
    _lcdMenu = lcdMenu;
}
...
String MeadeCommandProcessor::handleMeadeSetInfo(String inCmd)
{
    ...
    else if (inCmd[0] == 'L')  // Local time :SL19:33:03#
    {
        ...
        : SC: Calendar: If the date is valid 2 <string>s are returned, each string is 31 bytes long. The first is:
        "Updating planetary data#" followed by a second string of 30 spaces terminated by '#'
        */

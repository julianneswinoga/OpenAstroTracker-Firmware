...
// Configure the Focus driver.
    ...
    // Set the DEC limit position to the current stepper position. If upper is true, sets the upper limit, else the
    // lower limit.
...
// Auto Home with TMC2209 UART
    ...
    // Runs the RA motor at twice the speed (or stops it), or the DEC motor at tracking speed for the given duration in
    // ms.
  ...
  private:
    ...
    // STATUS_SLEWING_FREE, STATUS_TRACKING, STATUS_PARKING
    byte mountStatus();

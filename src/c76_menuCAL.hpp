...
enum
{
    ...
    HIGHLIGHT_LAST  = HIGHLIGHT_BACKLIGHT
        ...
        //     to slew east in half the time selected, then waits 1.5s and slews west in the same duration, and
        //     waits 1.5s.
...
bool processCalibrationKeys()
{
    ...
    if (calState == SPEED_CALIBRATION) {
        ...
        AzimuthMinutes
            = clamp(AzimuthMinutes, -60, 60);  // Only allow one arc hour at a time. Azimuth range is 2 arc hours
    }
        #endif
    ...
    if (checkForKeyChange && lcdButtons.keyChanged(&key)) {
        ...
        case DRIFT_CALIBRATION_WAIT: {
            ...
            if (key == btnSELECT) {
                // Take off 6s padding time. 1.5s start pause, 1.5s pause in the middle and 1.5s end pause and general
                // time slop. These are the times for one way. So total time is 2 x duration + 4.5s
        ...
        case DEC_UPPER_LIMIT_CONFIRM: {
            ...
            if (key == btnSELECT) {
                ...
                calState
                    = (calState == DEC_UPPER_LIMIT_CONFIRM) ? HIGHLIGHT_DEC_UPPER_LIMIT : HIGHLIGHT_DEC_LOWER_LIMIT;
                okToUpdateMenu = true;
            }
            else if (key == btnRIGHT) {

...
// counts steps, and is unaware of the microstep mode configured in TMC2209Stepper, which in turn affects angle moved
// per step. Therefore dynamically changing microstep mode causes errors when deriving angle from
// AccelStepper::position(). Even though the ULN2003 does not support microstepping, using different modes
// (full/half-step) between tracking/guiding & slewing has the problem. Consequently slewing, tracking, guiding now use
// the same microstep configuration regardless of mode. We plan to re-instate fine modes in a future release, but this
// will require a significant rework of the implementation.
//
...
// These settings work only with TMC2209 in UART connection (single wire to TX)
    ...
    //                  ^^^ leave at 0 for now, doesnt work properly yet
        ...
        #define RA_AUDIO_FEEDBACK                                                                                      \
            0  // If one of these are set to 1, the respective driver will shut off the stealthchop mode, resulting in a
               // audible whine
    #endif
    ...
    #ifndef USE_VREF
        #define USE_VREF                                                                                               \
            0  // By default Vref is ignored when using UART to specify rms current. Only enable if you know what you
               // are doing.
    #endif
...
// Theoretically correct RA tracking speed is 1.246586 (300 x 14.95903 / 3600) (V2 : 1.333800 (322 x 14.95903 / 3600)
// steps/sec (this is for 20T) Include microstepping ratio here such that steps/sec is updates/sec to stepper driver
...
// Enable Azimuth and Altitude motor functionality in Configuration.hpp
    ...
    // These settings work only with TMC2209 in UART connection (single wire to TX)
        ...
        #ifndef USE_VREF
            #define USE_VREF                                                                                           \
                0  // By default Vref is ignored when using UART to specify rms current. Only enable if you know what
                   // you are doing.
        #endif
    #endif
